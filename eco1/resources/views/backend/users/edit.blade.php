...
<div class="card">
    ...
    <div class="card-body">
      <form method="post" action="{{route('users.update',$user->id)}}">
        ...
        <input id="inputTitle" type="text" name="name" placeholder="Nhập"  value="{{$user->name}}" class="form-control">
        @error('name')
        ...
        <div class="form-group">
          ...
          <div class="form-group">
            ...
            <select name="status" class="form-control">
                <option value="active" {{(($user->status=='active') ? 'selected' : '')}}>Có</option>
                <option value="inactive" {{(($user->status=='inactive') ? 'selected' : '')}}>Ko</option>
            </select>
          @error('status')
          <span class="text-danger">{{$message}}</span>
          @enderror

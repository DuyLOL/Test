...
<div class="card">
  <h5 class="card-header">Cập nhật bình luận</h5>
  <div class="card-body">
    <form action="{{route('user.post-comment.update',$comment->id)}}" method="POST">
      ...
      <div class="form-group">
        ...
        <select name="status" id="" class="form-control">
          ...
          <option value="active" {{(($comment->status=='active')? 'selected' : '')}}>Trực tuyến</option>
          <option value="inactive" {{(($comment->status=='inactive')? 'selected' : '')}}>vô tuyến</option>
        </select>
      </div>
